(function(){
  // ============================================================
  // peregrine-mcp-app widget — MCP-UI rebuild of the
  // stream-blog-claude blog wizard.
  //
  // Architecture (model-orchestrated, mirrors stream-blog-claude):
  //   - Widget collects user input and posts plain-text PROMPTS to
  //     the host via MCP-UI postMessage `prompt` intent.
  //   - The model (Claude Desktop) reads the prompt, calls
  //     Stream-MCP / DOTCOM-MCP tools to do the actual work, then
  //     calls peregrine-mcp-app's `show_blog_wizard` tool to
  //     re-render the widget with updated state.
  //   - State is delivered via either:
  //       a) the URL query param ?state=<urlencoded-json>  (server-driven)
  //       b) the `data-state` attribute on #bw-root          (in case the
  //          host bakes it directly)
  //       c) sessionStorage backup (survives re-mounts inside the tab).
  // ============================================================
  ...
  function sendPrompt(prompt) { postToHost('prompt', { prompt: prompt }); }
  function openLink(url)      { postToHost('link', { url: url }); }
  ...
  const DEFAULT_DISABLED = ['brand-marks.md', 'color.md', 'imagery.md', 'lockups.md', 'templates-events-gear.md', 'typography.md'];
  ...
  const state = {
    ...
    skills:   { disabled: new Set(DEFAULT_DISABLED), custom: [] },
    brief:    { text: '', status: 'idle' },
    page:     { items: [], url: '', app_url: '', status: 'idle' },
  };
  let currentStep = -1;

  // ----- Session-scoped persistence -----
  const STORAGE_KEY = 'peregrine-wizard-v1';
  ...
  function clearSnapshot() { try { sessionStorage.removeItem(STORAGE_KEY); } catch (e) {} }
    ...
    files.forEach(file => {
      ...
      reader.onload = e => {
        ...
        if (--pending === 0) {
          ...
          if (list) added.forEach(s => list.insertAdjacentHTML('beforeend', skillRowHtml(s, false)));
          updateActiveBadge();
          saveSnapshot();
        }
      };
  ...
  // ----- Step 3: Brief Generation (model orchestrates Stream-MCP) -----
  // Same trigger contract as stream-blog-claude — embeds proposal + active
  // skills inline because steps 1 & 2 are client-side, so the model has no
  // prior data-state to read from on the first turn.
  ...
  window.bwCreatePage = function() {
    ...
    currentStep = 3;
    state.page = { items: [], url: '', app_url: '', status: 'streaming' };
    renderPageStreaming();
    saveSnapshot();
          ...
          `<div class="page-gen__success">` +
            ...
            `<div class="page-gen__success-body">` +
              ...
              `<div class="page-gen__url-row">` +
                ...
                (url ? `<a href="${escAttr(url)}" target="_blank" rel="noreferrer" class="page-gen__url-open" aria-label="Open page in new tab" onclick="bwOpenUrl(event, '${escAttr(url)}')">${SVG_EXT}</a>` : '') +
              `</div>` +
            `</div>` +
          `</div>` +
  ...
  window.bwOpenUrl = function(ev, url) { ev.preventDefault(); openLink(url); };
  ...
  window.bwReset = function() {
    ...
    state.skills   = { disabled: new Set(DEFAULT_DISABLED), custom: [] };
  ...
  // ----- Hydration from baked data-state -----
  ...
  if (snap) {
    if (bakedOk) {
      // Custom skill bodies are typically stripped from the URL state to keep
      // it under URL length limits — restore them from sessionStorage.
      if (!Array.isArray(state.skills.custom) || state.skills.custom.length === 0
          || state.skills.custom.every(s => !s.content)) {
