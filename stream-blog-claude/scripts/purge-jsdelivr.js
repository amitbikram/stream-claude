#!/usr/bin/env node
// Purges jsDelivr's edge cache for every asset in stream-blog-claude/artifacts/assets/.
// jsDelivr caches branch refs (@main) for ~12h; tag refs (@v1.x.x) never expire.
// Run this after pushing changes so the blog wizard picks up the new CSS/JS immediately.
//
...
const ASSETS_DIR = 'stream-blog-claude/artifacts/assets';

const ref = process.argv[2] || 'main';
const dir = join(__dirname, '..', 'artifacts', 'assets');
...
(async () => {
  console.log(`Purging ${paths.length} blog-wizard assets at @${ref}…`);
